...
MODEL_FILE_VELA = "/run/media/MICRO_YHC-sda1/lab_files/detect_vela.tflite"
LABEL_FILE = "/run/media/MICRO_YHC-sda1/lab_files/labels.txt"
RESULT_FILE = "/run/media/MICRO_YHC-sda1/lab_files/detection_result_with_NPU.jpg"

# Load the NPU delegate
ext_delegate = [
    tflite.load_delegate("/usr/lib/libethosu_delegate.so", {"device_name": "/dev/ethosu0"})
]

# Load the TFLite model with NPU delegate
interpreter = tflite.Interpreter(model_path=MODEL_FILE_VELA, experimental_delegates=ext_delegate)
...
# Handle input data based on expected type
if input_dtype == np.uint8:
    input_data = np.asarray(img, dtype=np.uint8)
elif input_dtype == np.float32:
    input_data = np.asarray(img, dtype=np.float32) / 255.0  # Normalize to [0, 1]
else:
